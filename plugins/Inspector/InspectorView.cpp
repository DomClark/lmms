...
Qt::ItemFlags PropertyTableModel::flags(const QModelIndex &index) const
{
	...
	if (index.column() == 1 && m_object->metaObject()->property(index.row()).isWritable())
	{
...
bool PropertyTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
	...
	{
		if (m_object->metaObject()->property(index.row()).write(m_object, value))
		{
...
void SelectorWidget::mouseReleaseEvent(QMouseEvent *event)
{
	...
	{
		hide();
		if (m_currentTarget) { emit objectSelected(m_currentTarget); }
	}
}
...
bool InspectorView::eventFilter(QObject *watched, QEvent *event)
{
	...
	case QEvent::ChildAdded:
	{
		...
		{
			// The object's constructor isn't yet complete, so process it later.
...
void InspectorView::updateTree()
{
	for (const auto object : m_incomingObjects)
	{
		...
		if (!object->parent()->property(INSPECTOR_PROPERTY).isValid()) { continue; }
		addObject(object);
	}
...
void InspectorView::addObject(QObject *object)
{
	...
	while (!stack.empty())
	{
		...
		std::copy_if(children.begin(), children.end(), std::back_inserter(stack), [](QObject *object)
			{
				return object->isWidgetType();
			});
		const auto parentItem = static_cast<QTreeWidgetItem *>(
			object->parent()->property(INSPECTOR_PROPERTY).value<void *>());
...
void InspectorView::removeObject(QObject *object)
{
	...
	while (!objectStack.empty())
	{
		...
		const auto &children = object->children();
		std::copy(children.begin(), children.end(), std::back_inserter(objectStack));
	...
	while (!treeStack.empty())
	{
		...
		treeStack.pop_back();
		for (auto i = 0; i < item->childCount(); ++i) { treeStack.push_back(item->child(i)); }
		const auto object = static_cast<QObject *>(
